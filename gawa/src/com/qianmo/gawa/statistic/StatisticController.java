...
public class StatisticController {
	...
	public ModelAndView index(HttpSession session,HttpServletResponse response) {
		...
		operlogService.addOperlogToDb(username, "浏览上网人员统计");
		return new ModelAndView("statistic/statisticuserlist");
	}
	
	@RequestMapping(value = "getdata", method = RequestMethod.POST)
	//public String logout(HttpSession session) {
	public void getdata(HttpSession session,HttpServletResponse response,
			Integer page,Integer rows,Integer id,String mobileno,
		...
		try {
			if(date1==null){
				result = statisticDao.findStatisticUserByPage(offset,rows);
				total= statisticDao.findStatisticUserCount();
	...
	public void exportTable(HttpSession session,HttpServletResponse response,
		...
		try {
			if(mobileno==null){
				result = statisticDao.findStatisticUserByPage(0,1000);
				total= statisticDao.findStatisticUserCount();
	...
	public LinkedHashMap<String, String> getLeadToFiledPublicQuestionBank() {  
	    ...
	    superClassMap.put("mobileno", "手机号");  
	    superClassMap.put("count", "上网次数");  
	    superClassMap.put("mac", "mac");  
	    superClassMap.put("imsi", "imsi");  
	    superClassMap.put("uid", "uid");  
