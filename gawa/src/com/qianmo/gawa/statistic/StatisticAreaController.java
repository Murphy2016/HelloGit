...
@Controller
@RequestMapping(value = "/statisticarea")
public class StatisticAreaController {
...
//	
	...
	public ModelAndView index(HttpSession session,HttpServletResponse response) {
		...
		operlogService.addOperlogToDb(username, "浏览场所统计信息");
		return new ModelAndView("statistic/statisticarealist");
	}
	...
	public void getdata(HttpSession session,HttpServletResponse response,
		...
		try {
			if(date1==null){
				result = statisticDao.findStatisticAreaByPage(offset,rows);
				total= statisticDao.findStatisticAreaCount();
			}else if( date1!=null && date2!=null){
				result = statisticDao.findStatisticAreaByDate(date1,date2);
				total= result.size();
			}
	...
	public void exportTable(HttpSession session,HttpServletResponse response,
		...
		try {
			if(mobileno==null){
				result = statisticDao.findStatisticAreaByPage(0,1000);
				total= statisticDao.findStatisticAreaCount();
		...
		try {
			ExcelUtil.listToExcel(result, fieldMap, "场所", response);
	...
	public LinkedHashMap<String, String> getLeadToFiledPublicQuestionBank() {  
	    ...
	    superClassMap.put("count", "上网服务场所数");  
	    superClassMap.put("working", "营业场所数");  
	    superClassMap.put("unworking", "停业场所数");  
