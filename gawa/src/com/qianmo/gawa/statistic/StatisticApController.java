...
@Controller
@RequestMapping(value = "/statisticap")
public class StatisticApController {
	...
	public ModelAndView index(HttpSession session,HttpServletResponse response) {
		...
		operlogService.addOperlogToDb(username, "浏览设备统计信息");
		return new ModelAndView("statistic/statisticaplist");
	}
	...
	public void getdata(HttpSession session,HttpServletResponse response,
		...
		try {
			if(date1==null){
				result = statisticDao.findStatisticApByPage(offset,rows);
				total= statisticDao.findStatisticApCount();
			}else if( date1!=null && date2!=null){
				result = statisticDao.findStatisticApByDate(date1,date2);
				total= result.size();
			}
	...
	public void getdata1(HttpSession session,HttpServletResponse response,
		...
		for(Object elem:result){
			Statistic e = (Statistic)elem;
			date.add(e.getDate1());
			count.add(e.getService_online());
			count1.add(e.getData_online());
			count2.add(e.getWorking());
		}
		HashMap map = new HashMap();
	...
	public void exportTable(HttpSession session,HttpServletResponse response,
		...
		try {
			if(mobileno==null){
				result = statisticDao.findStatisticApByPage(0,1000);
				total= statisticDao.findStatisticApCount();
		...
		try {
			ExcelUtil.listToExcel(result, fieldMap, "设备", response);
	...
	public LinkedHashMap<String, String> getLeadToFiledPublicQuestionBank() {  
	    ...
	    superClassMap.put("service_online", "服务在线数");  
	    superClassMap.put("service_offline", "服务离线数");  
	    superClassMap.put("data_online", "数据在线数"); 
	    superClassMap.put("date_offline", "数据离线数");  
	    superClassMap.put("working", "设备工作数");  
	    
	    superClassMap.put("maintain", "设备维护数");  
	    superClassMap.put("exception", "设备异常数");  
	    superClassMap.put("other", "其他"); 
